...
export default function ListaInspecoesCaldeirasPage() {
  const [inspecoes, setInspecoes] = useState<InspecaoCaldeiraResumo[]>([])
  ...
  const carregarInspecoes = useCallback(async () => {
    ...
    if (data) {
      const inspecoesComCliente = data.map((inspecao: any) => ({
        id: inspecao.id,
        tag: inspecao.tag,
        categoria_caldeira: inspecao.categoria_caldeira,
        ...
        cliente_razao_social: inspecao.caldeiras?.clientes?.razao_social,
        cliente_cnpj: inspecao.caldeiras?.clientes?.cnpj,
      }))
  ...
  async function excluirInspecao(id: string) {
    if (!confirm('Deseja realmente excluir esta inspeção de caldeira e todos os seus dados?')) return
    setExcluindo(id)
    const supabase = createClient()
    const { error } = await supabase.from('inspecoes_caldeiras').delete().eq('id', id)
    if (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-orange-600 flex items-center justify-center shadow-md flex-shrink-0">
            <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.362 5.214A8.252 8.252 0 0112 21 8.25 8.25 0 016.038 7.048 8.287 8.287 0 009 9.6a8.983 8.983 0 013.361-6.867 8.21 8.21 0 003 2.48z" />
            </svg>
          </div>
          <div>
            <h1 className="text-xl font-bold text-gray-900 tracking-tight">NR-13 — Caldeiras</h1>
            <p className="text-sm text-gray-500">Inspeção ASME Sec. I e IV, Auditoria Operacional e PMTA</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between bg-white rounded-xl border border-gray-200 px-4 py-3 shadow-sm">
        ...
        <Link href="/laudos/nr13/caldeiras/novo" className="btn-primary text-sm" style={{ backgroundColor: '#ea580c' }}>
          + Nova Inspeção Caldeira
        </Link>
      </div>
      ...
      {inspecoes.length === 0 ? (
        <div className="card flex flex-col items-center justify-center py-16 px-8 text-center">
          ...
          <h3 className="text-base font-semibold text-gray-900 mb-1">Nenhuma caldeira inspecionada ainda</h3>
          <p className="text-sm text-gray-500 mb-6 max-w-xs">
            Registre sua primeira inspeção de caldeira com cálculo de seção de espelhos e costado.
          </p>
          <Link href="/laudos/nr13/caldeiras/novo" className="btn-primary text-sm" style={{ backgroundColor: '#ea580c' }}>
        ...
        <div className="space-y-2">
          {inspecoes.map((insp) => (
            <div
              key={insp.id}
              className="bg-white rounded-xl border border-gray-200 shadow-sm hover:border-orange-200 hover:shadow-md transition-all duration-150 flex items-center gap-4 p-4 group cursor-pointer"
              onClick={() => router.push(`/laudos/nr13/caldeiras/${insp.id}`)}
            >
              {/* INFO */}
              <div className="flex-1 min-w-0">
                ...
                <p className="font-semibold text-gray-900 truncate group-hover:text-orange-700 transition-colors">
                  Caldeira Categoria {insp.categoria_caldeira ?? '?'}
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <span className="p-2 text-gray-300 group-hover:text-orange-500 transition-colors">
